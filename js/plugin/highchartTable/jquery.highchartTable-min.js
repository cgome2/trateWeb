  ...
  $.fn.highchartTable = function() {
    ...
    this.each(function() {
      ...
      var highChartConfig = {
		...
		if(this.point.name> 1000)
		{
		valor = this.point.name/1000;
		valor = Math.round(valor*100)/100;
    var parts=valor.toString().split(".");
    valor = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",") + (parts[1] ? "." + parts[1] : "");

		valor = valor + 'k'; 
		}
else
{
valor = this.point.name
}
                return '<strong>' + this.series.name + '</strong><br />' + xValue + ' : '  +valor;
              }
            }
        },
        credits: {
